...
void RecurrentLayer<Dtype>::printBlobByName(const std::string& blob_name){
  Blob<Dtype>* tmp_blob = unrolled_net_->blob_by_name(blob_name).get();
  if (tmp_blob == NULL){
      cout << blob_name << " is NULL" << endl;
      return;
  }
  for (int k = 0; k != tmp_blob->count(); ++k){
      if (k % 80 == 0)
          cout << endl;
      cout << tmp_blob->cpu_data()[k] << " ";
  }
  cout << endl;
}
...
// #ifdef QIQI_DEBUG
//     cout << "*******************\nrecurrent output\n*******************\n";
//     for (int j = 0; j != recur_output_blobs_[i]->count(); ++j){
//          cout << timestep_T_data[j] << " ";
//     }
//     cout << endl;
// #endif
  }
#ifdef QIQI_DEBUG
  cout << "******************************\n layer top before network forward\n****************************************\n";
  for (int i = 0; i != top.size(); ++i){
    cout << "====================================" << endl;
    for (int j = 0; j != top[i]->count(); ++j){
         cout << top[i]->cpu_data()[j] << " ";
    }
    cout << endl;
    cout << "====================================" << endl;
  }
  ...
  CHECK_EQ(top.size(), output_blobs_.size());
  for (int i = 0; i < top.size(); ++i) {
    cout << "top " << i << ": " << static_cast<const void*>(top[i]) << endl;
    cout << "output blobs " << i << ": " << static_cast<const void*>(output_blobs_[i]) << endl;
    cout << "top data " << i << ": " << static_cast<const void*>(top[i]->cpu_data()) << endl;
    cout << "output blobs data " << i << ": " << static_cast<const void*>(output_blobs_[i]->cpu_data()) << endl;
    cout << "top diff " << i << ": " << static_cast<const void*>(top[i]->cpu_diff()) << endl;
    cout << "output blobs diff " << i << ": " << static_cast<const void*>(output_blobs_[i]->cpu_diff()) << endl;
  }
#endif
  ...
  // }
#ifdef QIQI_DEBUG
  cout << "******************************\naddress analysis after forward\n****************************************\n";
  CHECK_EQ(top.size(), output_blobs_.size());
  for (int i = 0; i < top.size(); ++i) {
    cout << "top " << i << ": " << static_cast<const void*>(top[i]) << endl;
    cout << "output blobs " << i << ": " << static_cast<const void*>(output_blobs_[i]) << endl;
...
//       for (int j = 0; j != this->blobs_[i]->count(); ++j){
//           cout << this->blobs_[i]->cpu_data()[j] << " ";
//       }
//       cout << endl;
//   }
//   vector<string> recur_output_names;
...
//           sstream << j;
//           string tmp_str = sstream.str();
//           string recur_name = recur_output_names[i].substr(0, recur_output_names[i].size() - 1) + tmp_str;
//           cout << "**************\n" << recur_name << "\n***********************\n";
//           Blob<Dtype>* tmp_blob = unrolled_net_->blob_by_name(recur_name).get();
//           for (int k = 0; k != tmp_blob->count(); ++k){
//               cout << tmp_blob->cpu_data()[k] << " ";
//           }
//           cout << endl;
...
// #endif
...
#ifdef QIQI_DEBUG
  cout << "******************************\n net input\n****************************************\n";
  for (int i = 0; i != unrolled_net_->input_blobs().size(); ++i){
      cout << "====================================" << endl;
      for (int j = 0; j != unrolled_net_->input_blobs()[i]->count(); ++j){
          cout << unrolled_net_->input_blobs()[i]->cpu_data()[j] << " ";
      }
      cout << endl;
      cout << "====================================" << endl;
  }
  cout << "******************************\n net output\n****************************************\n";
  for (int i = 0; i != unrolled_net_->output_blobs().size(); ++i){
      cout << "====================================" << endl;
      for (int j = 0; j != unrolled_net_->output_blobs()[i]->count(); ++j){
          cout << unrolled_net_->output_blobs()[i]->cpu_data()[j] << " ";
      }
      cout << endl;
      cout << "====================================" << endl;
  }
  ...
  //   string h_name = "h_" + tm1s;
  //   string cont_name = "cont_" + ts;
  //   string h_conted_name = "h_conted_" + tm1s;
  //   cout << "====================================" << endl;
  //   cout << h_name << endl;
  //   printBlobByName(h_name);
  //   cout << cont_name << endl;
  //   printBlobByName(cont_name);
  //   cout << h_conted_name << endl;
  //   printBlobByName(h_conted_name);
  //   cout << "====================================" << endl;
  // }
  cout << "******************************\n lstm unit\n*********************************\n";
  for (int t = 1; t <= this->T_; ++t){
    string tm1s = this->int_to_str(t - 1);
    string ts = this->int_to_str(t);
    std::vector<string> blobnames;
    blobnames.push_back("c_" + tm1s);
    blobnames.push_back("gate_input_" + ts);
    blobnames.push_back("cont_" + ts);
    blobnames.push_back("c_" + ts);
    blobnames.push_back("h_" + ts);
    cout << "====================================" << endl;
    for (int i = 0; i != blobnames.size(); ++i){
         cout << blobnames[i] << endl;
         printBlobByName(blobnames[i]);
    }
    cout << "====================================" << endl;
  }
  cout << "====================================" << endl;
